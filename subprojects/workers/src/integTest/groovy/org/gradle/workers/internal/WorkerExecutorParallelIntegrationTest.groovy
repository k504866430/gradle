...
class WorkerExecutorParallelIntegrationTest extends AbstractWorkerExecutorIntegrationTest {
    ...
    def "multiple work items can be executed in parallel in #isolationMode (wait for results: #waitForResults)"() {
        ...
        'IsolationMode.THREAD'  | true
        'IsolationMode.THREAD'  | false
    }
    ...
    def "a second task action does not start until all work submitted by a previous task action is complete"() {
        ...
        buildFile << """
            task parallelWorkTask(type: MultipleWorkItemTask) {
                ...
                doLast { submitWorkItem("taskAction2", runnableClass) { isolationMode = IsolationMode.THREAD } }
                ...
                doLast { submitWorkItem("taskAction5", runnableClass) { isolationMode = IsolationMode.THREAD } }
                doLast { submitWorkItem("taskAction6", runnableClass) { isolationMode = IsolationMode.THREAD } }
            }
        """
        ...
        buildFile << """
            ...
            task parallelWorkTask(type: MultipleWorkItemTask) {
                doLast { 
                    ...
                    submitWorkItem("workItem3", runnableClass) { isolationMode = IsolationMode.THREAD } 
                }
            }
        """
    ...
    def "all errors are reported when submitting failing work in #isolationModeDescription"() {
        ...
        'IsolationMode.THREAD'  | 'IsolationMode.THREAD'  | 'IsolationMode.THREAD'
        'IsolationMode.PROCESS' | 'IsolationMode.THREAD'  | 'both IsolationMode.PROCESS and IsolationMode.THREAD'
        'IsolationMode.NONE'    | 'IsolationMode.THREAD'  | 'both IsolationMode.NONE and IsolationMode.THREAD'
    }
